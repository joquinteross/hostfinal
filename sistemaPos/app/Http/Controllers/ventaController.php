...
class ventaController extends Controller
{
    ...
    {
        $ventas = Venta::with(relations: ['comprobante', 'cliente.persona', 'user'])
            ->where('estado', 1)
            ->latest()
            ->get();
    ...
    {
    $venta = Venta::with(['cliente.persona', 'comprobante', 'productos'])->findOrFail($id);
    
    $pdf = Pdf::loadView('venta.factura', compact('venta'));
